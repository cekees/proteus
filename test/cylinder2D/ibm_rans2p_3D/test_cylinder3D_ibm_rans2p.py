...
#         self.example_setting("T=8.0 vspaceOrder=2 onlySaveFinalSolution=True")
    ...
    # "test.cylinder2D.ibm_rans2p_3D", matching what pytest's
    # --import-mode=importlib actually resolves this test module's package
    # to) -- simulation now runs and compares against the real baseline, but
    # ALL 276/276 elements differ, several off by roughly an order of
    # magnitude (e.g. 0.126 vs 1.668, 0.334 vs 1.673). This looks like a real
    # bug, not numerical drift -- needs review before trusting.
    def test_ex2(self):
