#!/usr/bin/env bash
#
# test_3 -- Szymkiewicz (2009) Fig. 6, 5 m column, dz = 50 cm.
#
...
# add it with SCHEMES="stab_0 stab_2 FCT" if you want it anyway.
#
#   ./run_test_3.sh                       # stab_2 + FCT, then the figure
#   SCHEMES="FCT" ./run_test_3.sh         # one scheme
#   PLOT_ONLY=1 ./run_test_3.sh           # re-draw from existing archives
#
set -u
...
for scheme in $SCHEMES; do
    ...
    (
        ...
        exit "${PIPESTATUS[0]}"
    ) || echo "!!! test_3/$scheme FAILED -- see $rundir/run.log" >&2
done

echo "=== figure ==="
SCHEMES="$SCHEMES" $PLOT_PY "$HERE/plot_test_3.py"
