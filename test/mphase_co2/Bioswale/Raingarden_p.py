...
coefficients = mphase_co2.Coefficients(nd,
                                     ...
                                     STABILIZATION_TYPE= 2, #2,
                                     ENTROPY_TYPE=1,
                                     # Phase B Step 3e.1b: 'A' = Richards form (default),
                                     # 'B' = saturation-based wetting (couples (0,1) Jac).
                                     FORMULATION='A',
...
# --- Component 1 (S_w) IC/BC for Phase B Step 1 ---
# Trivial gas equation d(S_w)/dt = 0 with S_w = 1 everywhere.
# No Dirichlet (let S_w float; equation will hold it at IC).
# No advective/diffusive contribution (no entry in coefficient dicts).
def getDBC_2D_S_w(x, flag):
    return None

class ConstantOneIC_S_w:
    def uOfXT(self, x, t):
        return 1.0
