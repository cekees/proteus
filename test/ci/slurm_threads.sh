...
num_procs=(64 128 256 512 1024 2048 4096 5376) 
...
while [ $i -lt ${#num_nodes[*]} ]; do
    ...
    sed -e "s/NUM_NODES/${num_nodes[$i]}/g" -e "s/TASKS_PER_NODE/${num_procs[$i]}/g" -e "s/NUM_REFINEMENT/$(($i+3))/g" -e "s/PARTITION_NAME/${part_name[$i]}/g" poisson.sh > poisson_slurm.sh 
