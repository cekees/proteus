#!/usr/bin/env python3
"""
Plot Richards grad(u) snapshots for Stab_0 and Stab_2 at selected times.
...
try:
    ...
    raise SystemExit(
        "matplotlib is required to run plot_grad_u.py. "
...
OUTPUT_FILE = HERE / "grad_u_selected_times_compare.png"
MARKER_SIZE = 4.0
ARROW_WIDTH = 0.004
POINT_ALPHA = 0.18
QUIVER_SCALE = None
...
def load_profile_index(base_dir: Path, stab_tag: str) -> dict[float, Path]:
    files = {}
    pattern = f"richards_q_grad_u_profile_{stab_tag}_t*.txt"
...
def compute_shared_scale(snapshots: list[np.ndarray]) -> float:
    ...
    q95 = np.percentile(all_norms, 95.0)
...
def sort_snapshot(data: np.ndarray) -> np.ndarray:
...
if TARGET_TIMES:
    fig.suptitle("Richards grad(u) comparison near selected common times", fontsize=15)
else:
    fig.suptitle("Richards grad(u) comparison at last common time", fontsize=15)

fig.tight_layout(rect=(0.0, 0.0, 0.94, 0.97))
cbar = fig.colorbar(mappable, ax=axes.ravel().tolist(), fraction=0.025, pad=0.02)
cbar.set_label("|grad(u)|")
...
for target_t, actual_t in selected:
    snap0 = load_snapshot(files0[actual_t])
    snap2 = load_snapshot(files2[actual_t])
    snap0_sorted = sort_snapshot(snap0)
    snap2_sorted = sort_snapshot(snap2)
    if snap0_sorted.shape == snap2_sorted.shape and np.allclose(snap0_sorted[:, :3], snap2_sorted[:, :3]):
        delta = snap2_sorted[:, 3:5] - snap0_sorted[:, 3:5]
        delta_norm = np.linalg.norm(delta, axis=1)
        print(f"target t={target_t:.6f} -> actual common t={actual_t:.9e}")
        print(f"  max |grad(u)_stab2 - grad(u)_stab0| = {delta_norm.max():.9e}")
        print(f"  rms |grad(u)_stab2 - grad(u)_stab0| = {np.sqrt(np.mean(delta_norm**2)):.9e}")
    else:
        print(f"target t={target_t:.6f} -> actual common t={actual_t:.9e}")
        print("  Coordinate ordering differs between the two snapshots; skipping delta statistics.")
