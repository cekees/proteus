...
public:
    ...
    for (int eN = 0; eN < nElements_global; eN++) {
      ...
      for (int k = 0; k < nQuadraturePoints_element; k++) {
        ...
        for (int i = 0; i < nDOF_test_element; i++) {
          ...
          elementResidual_u[i] += ck.Mass_weak(m_t, u_test_dV[i]) + ck.Advection_weak(f, &u_grad_test_dV[i_nSpace]) + ck.Diffusion_weak(a_rowptr.data(), a_colind.data(), a, grad_u, &u_grad_test_dV[i_nSpace]) + VMS * ck.SubgridError(subgridError_u, Lstar_u[i]) + VMS * ck.NumericalDiffusion(q_numDiff_u[eN_k], grad_u, &u_grad_test_dV[i_nSpace]);
        } //i
        //
        q_m.data()[eN_k] = m;
        q_u.data()[eN_k] = u;
        ...
        for (int i = 0; i < nDOF_test_element; i++) {
          ...
          for (int j = 0; j < nDOF_trial_element; j++) {
            ...
            elementJacobian_u_u[i][j] += ck.MassJacobian_weak(dm_t, u_trial_ref.data()[k * nDOF_trial_element + j], u_test_dV[i]) + ck.AdvectionJacobian_weak(df, u_trial_ref.data()[k * nDOF_trial_element + j], &u_grad_test_dV[i_nSpace]) +
                                         ck.DiffusionJacobian_weak(a_rowptr.data(), a_colind.data(), a, da, grad_u, &u_grad_test_dV[i_nSpace], 1.0, u_trial_ref.data()[k * nDOF_trial_element + j], &u_grad_trial[j_nSpace]) + VMS * ck.SubgridErrorJacobian(dsubgridError_u_u[j], Lstar_u[i]) + VMS * ck.NumericalDiffusionJacobian(q_numDiff_u[eN_k], &u_grad_trial[j_nSpace], &u_grad_test_dV[i_nSpace]);
          } //j
        } //i
      } //k
      //
  ...
  void FCTStep(arguments_dict &args)
  {
    ...
    for (int i = 0; i < numDOFs; i++) {
      ...
      for (int offset = csrRowIndeces_DofLoops.data()[i]; offset < csrRowIndeces_DofLoops.data()[i + 1]; offset++) {
        ...
        //double sdotj = (solH.data()[j] - soln.data()[j]);
        double sdotj = uDotLow[j];
        alpha_fA     = ((FluxCorrectionMatrix[ij] > 0) ? fmin(Rposi, Rneg[j]) : fmin(Rnegi, Rpos[j])) * FluxCorrectionMatrix[ij];
  ...
  void calculateResidual_entropy_viscosity(arguments_dict &args)
  {
    ...
    for (int i = 0; i < numDOFs; i++) {
      ...
      evaluateCoefficients(a_rowptr.data(), a_colind.data(), rho, beta, gravity.data(),
                           alpha.data()[elementMaterialTypes.data()[0]], //cek hack, only for 1 material
                           n.data()[elementMaterialTypes.data()[0]], thetaR.data()[elementMaterialTypes.data()[0]], thetaSR.data()[elementMaterialTypes.data()[0]], &KWs.data()[elementMaterialTypes.data()[0] * nnz], u_free_dof_old[i], mn, dmn, fn, dfn, an, dan, asn, Krn, dKrn);
      sn.data()[i]   = mn;
      sLow.data()[i] = m;
      ...
      if (false) {
        ...
        double mMin = rho * thetaR.data()[elementMaterialTypes.data()[0]];
        double mMax = rho * (thetaR.data()[elementMaterialTypes.data()[0]] + thetaSR.data()[elementMaterialTypes.data()[0]]);
        if (m < mMin || m > mMax) { std::cout << "mass out of bounds " << mMin << '\t' << m << '\t' << mMax << std::endl; }
        evaluateInverseCoefficients(a_rowptr.data(), a_colind.data(), rho, beta, gravity.data(),
                                    alpha.data()[elementMaterialTypes.data()[0]], //cek hack, only for 1 material
                                    n.data()[elementMaterialTypes.data()[0]], thetaR.data()[elementMaterialTypes.data()[0]], thetaSR.data()[elementMaterialTypes.data()[0]], &KWs.data()[elementMaterialTypes.data()[0] * nnz], uLow.data()[i], m, dm, f, df, a, da);
      }
  ...
  void invert(arguments_dict &args)
  {
    ...
    for (int i = 0; i < numDOFs; i++) {
      ...
      if ((*input_solution).data()[i] < mMin - 0.001 || (*input_solution).data()[i] > mMax + 0.001) { std::cout << "mass out of bounds " << mMin << '\t' << (*input_solution).data()[i] << '\t' << mMax << std::endl; }
