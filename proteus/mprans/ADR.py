...
from proteus.NonlinearSolvers import ExplicitLumpedMassMatrix,ExplicitConsistentMassMatrixForADR,TwoStageNewton
from proteus import TimeIntegration
from proteus.mprans.cADR import *
from proteus import *
...
class ShockCapturing(ShockCapturing_base):
    def __init__(self,
        ...
        self.nSteps = 0
        if self.lag:
            logEvent("ADR.ShockCapturing: lagging requested but must lag the first step; switching lagging off and delaying")
    ...
    def updateShockCapturingHistory(self):
        ...
        if self.lag == False and self.nStepsToDelay is not None and self.nSteps > self.nStepsToDelay:
            logEvent("ADR.ShockCapturing: switched to lagged shock capturing")
            self.lag = True
            self.numDiff_last = []
            for ci in range(self.nc):
                self.numDiff_last.append(self.numDiff[ci].copy())
        logEvent("ADR: max numDiff %e" % (globalMax(self.numDiff_last[0].max()),))
...
class Coefficients(proteus.TransportCoefficients.TC_base):
    from proteus.ctransportCoefficients import ADRCoefficientsEvaluate
    from proteus.ctransportCoefficients import VolumeAveragedADRCoefficientsEvaluate
    ...
    def preStep(self, t, firstStep=False):
        ...
        if self.checkMass:
            ...
            logEvent("Phase  0 mass before ADR step = %12.5e" % (self.m_pre,), level=2)
        copyInstructions = {}
    ...
    def postStep(self, t, firstStep=False):
        ...
        if self.checkMass:
            self.m_post = Norms.scalarDomainIntegral(self.model.q['dV'],
                                                     self.model.q[('m', 0)],
                                                     self.model.mesh.nElements_owned)
            logEvent("Phase  0 mass after ADR step = %12.5e" % (self.m_post,), level=2)
            # self.fluxIntegral = Norms.fluxDomainBoundaryIntegral(self.model.ebqe['dS'],
            #                                                     self.model.ebqe[('advectiveFlux',0)],
            #                                                     self.model.mesh)
            #logEvent("Phase  0 mass flux boundary integral after ADR step = %12.5e" % (self.fluxIntegral,),level=2)
            #logEvent("Phase  0 mass conservation after ADR step = %12.5e" % (self.m_post - self.m_last + self.model.timeIntegration.dt*self.fluxIntegral,),level=2)
    ...
    def evaluate(self, t, c):
        # mwf debug
        # print "ADRcoeficients eval t=%s " % t
        if c[('f', 0)].shape == self.q_v.shape:
            ...
            porosity = None
        if v is not None:
            # self.ADRCoefficientsEvaluate(self.eps,
            #                              v,
            #                              phi,
            #                              c[('u',0)],
            #                              c[('m',0)],
            ...
            #                              c[('f',0)],
            #                              c[('df',0,0)])
            self.VolumeAveragedADRCoefficientsEvaluate(self.eps,
                                                       v,
                                                       phi,
                                                       porosity,
                                                       c[('u', 0)],
...
class LevelModel(proteus.Transport.OneLevelTransport):
    ...
    def __init__(self,
        ...
        if self.coefficients.STABILIZATION_TYPE > 1:
            assert self.timeIntegration.isSSP == True, "If STABILIZATION_TYPE>1, use RKEV timeIntegration within ADR model"
            cond = 'levelNonlinearSolver' in dir(options) and (options.levelNonlinearSolver ==
                                                               ExplicitLumpedMassMatrix or options.levelNonlinearSolver == ExplicitConsistentMassMatrixForADR)
            assert cond, "If STABILIZATION_TYPE>1, use levelNonlinearSolver=ExplicitLumpedMassMatrix or ExplicitConsistentMassMatrixForADR"
        ...
        if (self.coefficients.STABILIZATION_TYPE <= 1):  # SUPG or Taylor Galerkin
            self.calculateResidual = self.adr.calculateResidualElementBased            
        else:
            self.calculateResidual = self.adr.calculateResidualEdgeBased
